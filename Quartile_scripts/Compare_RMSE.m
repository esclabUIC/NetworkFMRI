...
% Set Directories 
dirs.Univariate = fullfile('../results/UnivariateQuartile_Cntrl4Three');
dirs.MVPA = fullfile('../results/MVPAQuartile_Cntrl4Three');

roi_names = {'Mentalizing','MPFC','PMC','RTP','LTP','RTPJ','LTPJ','Striatum','V1'};
...
for rr = 1:length(roi_names)
    ...
    fprintf('%s: RMSE_ROI = %0.3f, RMSE_MVPA = %0.3f, t(%i)=%0.3f, p=%0.3f\n',...
        roi_names{rr},mean(allRMSE{rr,1}(:,1)),mean(allRMSE{rr,1}(:,2)),stats.df,stats.tstat,P)
end

% Null Model
Y = repmat([1,2,3,4],30,1);
Y_Null = repmat([2,2,2,2],30,1);
