...
# mask_dir
mask_dir=../../masks/Neurosynth_${this_mask}

orig_file=$mask_dir/Neurosynth_${this_mask}.nii.gz
in_file=$mask_dir//Neurosynth_${this_mask}_hack.nii.gz

# Bash can't do floating point arithmetic - so hacking it by multiplying it by 10...
fslmaths $orig_file -mul 10 $in_file

# Loop over parcels
all_parc=($(seq 10 10 2000))
...
for parc in "${all_parc[@]}"
    ...
    interm_file=${mask_dir}/Neurosynth_${this_mask}_$((parc/10)).nii.gz
